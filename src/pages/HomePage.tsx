...
export default function HomePage() {
  ...
  const categories = [
    ...
    { name: 'Catering', icon: '🍽️', popular: false },
    { name: 'Cleaning', icon: '🧹', popular: true, highlighted: true },
    { name: 'Computer Fix', icon: '💻', popular: false },
    { name: 'Delivery', icon: '📦', popular: false },
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 sm:gap-6 mb-12">
            ...
            {categories.map((category, index) => (
              <motion.div
                ...
                whileHover={{
                  scale: category.highlighted ? 1.12 : 1.1,
                  y: -8,
                  transition: { duration: 0.2 }
                }}
                whileTap={{ scale: 0.95 }}
              ...
              >
                ...
                <motion.div
                  className="text-3xl sm:text-4xl mb-2 sm:mb-3 relative z-10 flex items-center justify-center h-12 w-12 mx-auto leading-none select-none"
                  animate={category.highlighted ? { y: [0, -3, 0] } : {}}
                  transition={category.highlighted ? { duration: 2.5, repeat: Infinity, ease: 'easeInOut' } : {}}
